create table if not exists Role(role varchar(255));
insert into Role(role) values ('CONSUMER');
insert into Role(role) values ('SELLER');

create table Categories(categoryName varchar(255));
...
create table Cart(totalAmount double, userId int);
insert into Cart(totalAmount, userId) values (20,1);
insert into Cart(totalAmount, userId) values (0,2);

create table UserRole(userId int, role varchar(255));
...
create table Product(price double, productName varchar(255), categoryId int, sellerId int);
...
create table CartProduct(cartId int, productId int, quantity int);
